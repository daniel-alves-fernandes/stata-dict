...
program define dict_define
  ...
  frame `2'{
    if ("`from'" != ""){
      if ("`type'" == "csv") qui: import delim "`from'", ///
                                  varn(1) case(preserve) enc("utf-8")
...
program define dict_list
  ...
  confirm frame `2'
  noisily frame `2': list `cols' `if' `in', clean compress
end
...
program define dict_getindex, rclass
  ...
  if (`r(N)' == 0){
    noisily: display as text "no matched observations in {bf:`2'}"
    exit
  }

  if inlist("`match'","","unique"){
    if (`r(N)' == 1){
      noisily: display as error `r(levels)'
      return scalar i = `r(levels)'
    }
    else{
      noisily: display as error "multiple matched observations in {bf:`2'}"
      exit
    }
  }
  else if ("`match'" == "first"){
...
program define dict_getvalues, sclass
  syntax namelist(min=2 max=2) [if] [in], [Match(string)]
  tokenize `namelist'
  ...
  if (`r(N)' == 0){
    noisily: display as error "no matched observations in {bf:`2'}"
    exit
  }
